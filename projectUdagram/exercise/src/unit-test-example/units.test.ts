...
describe ('concat', () => {
  it ("should concat 'dog' and 'cat'", () => {
    const result = concat('dog','cat');
    expect(result).to.equal('dogcat');
    
  });

  it ("should concat 'my' and 'name'", () => {
    const result = concat('my','name');
    expect(result).to.equal('myname');

  });

  it("should throw an error if ''", () => {
    expect(()=>{ concat ('my', '') }).to.throw('empty string')
  });
